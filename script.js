      ...
      fetch(`https://api.covid19api.com/live/country/${mapbox_api_country_name}/status/confirmed/date/${d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + (d.getDate())}`)
        .then(function (response) {
          ...
          if (response.status >= 200 && response.status < 300) {
            ...
            throw new Error('When fetching data from covid19api.com',{ cause: response });
          }
        })
        .then(function (json) {
          ...
          if(json.length === 0) {
            throw new Error('No data returned from covid19api.com',{ cause: 'Empty response from covid19api.com' });
          }
          ...
          if(err.cause){
            // Error while fetching from API server
            show_covid19api_error(err)
          }
          else{
            // Error in the 'then' section of the fetch request
...
function show_covid19api_error(msg){
  console.log(msg);
  document.querySelectorAll('#covid_data_table .cards').forEach(card => {
    card.innerHTML = msg
  });
}
